  ...
  return (
    <div className="w-full h-full flex flex-col">
      ...
      {!connectionId ? (
        ...
        <div className="w-full h-full bg-gray-100 dark:bg-black flex flex-col overflow-hidden font-mono text-[13px]">
          {/* History panel */}
          {showHistory && (
            <div className="flex-shrink-0 bg-gray-200 dark:bg-gray-950 border-b border-gray-300 dark:border-white/10 p-1.5">
              <div className="flex items-center justify-between mb-1.5">
                <span className="text-[11px] text-gray-600 dark:text-gray-400 font-medium">{t('redis.commandHistory')}</span>
                <button
              ...
              <div className="max-h-40 overflow-y-auto space-y-1">
                {[...history].reverse().map((item, index) => (
                  <button
                    ...
                    className="w-full text-left px-2 py-1 text-[11px] text-gray-700 dark:text-gray-300 hover:bg-gray-300 dark:hover:bg-gray-700 rounded truncate"
                  >
                    {item.command}
                  </button>
                ))}
          ...
          <div
            ref={outputRef}
            className="flex-1 overflow-y-auto p-2 space-y-1.5 bg-white dark:bg-gray-900"
          ...
          >
            {output.map((item, index) => (
              <div key={index} className="space-y-1">
                    ...
                    {item.data ? (
                      <div className="space-y-1">
                        ...
                        {Array.isArray(item.data) && item.data.length > 0 && item.data.some((d: any) => d.className) ? (
                          <div className="ml-1.5 space-y-1 border-l-2 border-green-400 dark:border-green-600 pl-1.5">
                            {item.data.map((d: any, i: number) => (
                              <div key={i} className="text-[11px]">
                                <div className="text-yellow-600 dark:text-yellow-400 font-medium">[{i}] {d.className || 'Unknown'}</div>
                                {d.data && (
                                  <pre className="mt-1 text-gray-600 dark:text-gray-300 whitespace-pre-wrap break-words">
                          ...
                          <pre className="ml-1.5 mt-1 border-l-2 border-green-400 dark:border-green-600 pl-1.5 text-[11px] text-gray-600 dark:text-gray-300 whitespace-pre-wrap break-words">
                            {formatJavaObject(item.data)}
                          </pre>
                        ) : typeof item.data === 'string' ? (
                          <pre className="ml-1.5 mt-1 border-l-2 border-blue-400 dark:border-blue-600 pl-1.5 text-[11px] text-gray-600 dark:text-gray-300 whitespace-pre-wrap break-words">
                            {item.data}
                          </pre>
                ...
                {/* Error output */}
                {item.type === 'error' && (
                  <div className="text-red-500 dark:text-red-400 whitespace-pre-wrap break-words">
                    {item.content}
                  </div>
                )}
              </div>
          ...
          {showAutocomplete && autocompleteSuggestions.length > 0 && (
            <div className="flex-shrink-0 bg-gray-200 dark:bg-gray-950 border-t border-gray-300 dark:border-white/10 p-1">
              <div className="text-[11px] text-gray-500 dark:text-gray-500 mb-1 px-1">{t('redis.pressTabAutocomplete')}</div>
          ...
          {/* Input area */}
          <div className="flex-shrink-0 border-t border-gray-300 dark:border-white/10 py-1.5 px-2 bg-gray-50 dark:bg-gray-900">
            <div className="flex items-center gap-1.5">
              ...
              {history.length > 0 && (
                <button
                  ...
                  className={`p-1 transition-colors text-[11px] ${
                    showHistory
